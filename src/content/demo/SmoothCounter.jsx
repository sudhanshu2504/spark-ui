...
function Counter({ from = 0, to, duration = 2000, prefix = '', suffix = '', label, delay = 0 }) {
  ...
  return (
    <motion.div
      ...
      className="group relative bg-zinc-950 border border-white/[0.06] rounded-2xl p-8 flex flex-col
    ...
    >
      <div className="w-1.5 h-1.5 rounded-full bg-yellow-400 mb-8
                      shadow-[0_0_10px_3px_rgba(234,179,8,0.5)]" />
      <span className="text-6xl font-black font-primary
                       bg-gradient-to-b from-white via-white to-zinc-400
                       bg-clip-text text-transparent leading-none">
        {prefix}{formatValue(displayValue, to)}{suffix}
      </span>
      <p className="text-xs text-zinc-500 mt-4 tracking-widest uppercase font-medium">
        {label}
      </p>
    </motion.div>
  );
...
export default function SmoothCounter() {
  return (
    <div className="h-full w-full bg-code flex flex-col items-center justify-center p-8">
      <div className="text-center mb-10">
        <span className="text-[11px] uppercase tracking-[0.2em] text-zinc-500 font-medium block mb-3">
          By the numbers
        </span>
